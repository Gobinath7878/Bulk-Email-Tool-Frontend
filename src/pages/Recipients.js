...
function Recipients() {
  ...
  const handleAddRecipient = async (event) => {
    ...
    try {
      const response = await api.post("/api/v1/recipient/create", { email: newRecipient });
  ...
  const handleUpdateRecipient = async (event) => {
    event.preventDefault();
    try {
      const response = await api.put(`/api/v1/recipient/put/${editingRecipientId}`, {
  ...
  return (
    ...
    <Container className="teacher-container p-5 mt-3">
      <Row>
        <Col lg='10'>    
    <div>
    <h2 className='fw-bold'><sup><i className="fa-solid fa-quote-left"></i></sup><span>Connecting Students,</span> <br/>Empowering Education: Stay in Touch with Our Email Hub!</h2>
      ...
      <div className="input-container">
        ...
        <button type="submit" className="button-send w-25 mx-2">Add</button>
        </div>
      </form>
      <p><span className="text-danger">*</span>It's important to ensure that the student's email address is correct, as this will be the primary way of communicating with them through the school website.</p>
    </div>
    ...
    <Col lg='2' className="mt-2">
      <div>
    <Button className="mx-2 p-2 button fw-bold"
 onClick={handleShow}>
              <i className="fa-solid fa-envelope text-success mx-1"></i> Mail Form
...
<Container className="mt-3 p-0">
  ...
  <tbody>
            ...
            <form onSubmit={handleUpdateRecipient}>
              <label>
                ...
                <input
                  ...
                  onChange={(event) => setEditedRecipient(event.target.value)}
                />
              </label>
              <Button type="submit" variant="success" className="mx-2">Save</Button>
              <Button
                type="button"
                variant="warning"
                className="mx-2"
    ...
    <Col lg='3'>
            ...
            <h1>Weekend Student's Circular</h1>
	<p>Dear Students,</p>
	...
	<p>If you are not able to attend this special course in these weekend classes, please let us know by emailing us at mailto:chego@example.com We will provide you with the necessary materials and resources for teachin these classes in online.</p>
